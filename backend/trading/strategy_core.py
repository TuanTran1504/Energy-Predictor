...
def validate_ai_trade_decision(decision: dict, context: dict,
    ...
    if setup_code not in ("A", "D"):
        return False, f"unsupported setup {setup_code!r}; only Setup A/D allowed"
...
def build_trade_plan(signal: str, setup_name: str, context: dict,
    ...
    if setup_code not in ("A", "D"):
        return None, f"unsupported setup {setup_code!r}; only Setup A/D allowed"
